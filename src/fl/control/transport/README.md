# fl/control/transport - JSON-RPC Transport Layer
...
Factory functions compose the transport layer with the JSON-RPC protocol layer (from `fl/control/rpc/protocol.h`) to create `RequestSource` and `ResponseSink` callbacks for `fl::Remote`.
...
The JSON-RPC specific functions have moved to `fl/control/rpc/protocol.h`:
...
```cpp
#include "fl/control/rpc/protocol.h"
